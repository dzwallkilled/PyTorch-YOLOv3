...
def detect_dataloader(model, dataloader, opt):
    ...
    for img_i, (path, detections) in enumerate(zip(images, img_detections)):
...
def detect_images(model, images, opt, output_root=''):
    ...
    for img_id, img_file in enumerate(images):
        ...
        for i, patch in img_patches.items():
            ...
            out_file = os.path.join(output_root, name + f'_patch{i:02d}.' + ext)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_def", type=str, default="config/rip/rip_model/yolov3-rip-level2-detect.cfg", help="path to model definition file")
    ...
    opt.checkpoint_model = '/data2/data2/zewei/exp/RipData/YOLOv3/patches/level2/CV5-1/checkpoints/yolov3_ckpt_100.pth'
    print(opt)

    device = get_available_device(1)
    ...
    # imgs, img_detections = detect_dataloader(model, dataloader, opt)
    images = ['samples/img_cv.png']
    detect_images(model, images, opt)
